...
<table class="table">
	...
	<tr>
		...
		<?php 
			$x=mysql_query("select sum(total_harga*jumlah) as total from barang_laku");	
	...
	<tr>
		...
		<?php 
			$x=mysql_query("select sum(laba*jumlah) as total from barang_laku");	
			$xx=mysql_fetch_array($x);			
			echo "<td><b> Rp.". number_format($xx['total']).",-</b></td>";
		?>
	</tr>
			...
			<div class="modal-header">
				...
				<div class="modal-body">				
					<form action="barang_laku_act.php" method="post">
						...
						<div class="form-group">
							<label>Nama Barang (Harga Satuan)</label>								
							<select class="form-control" name="nama">
								<?php 
								$brg=mysql_query("select * from barang");
								while($b=mysql_fetch_array($brg)){
									?>	
									<option value="<?php echo $b['nama']; ?>"><?php echo $b['nama'] ?> (Rp.<?php echo $b['harga'] ?>)</option>
									<?php 
								}
								?>
							</select>
